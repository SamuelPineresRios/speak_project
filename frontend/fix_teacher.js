      ...
      <div className="max-w-4xl mx-auto relative z-10 space-y-8 mt-12">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 glass-effect p-6 rounded-2xl border border-cyan-500/20">
          <div className="space-y-1">
            ...
            <p className="text-slate-400 font-mono text-sm flex items-center gap-2 mt-1">
              <Terminal className="w-3.5 h-3.5" /> Root Admin: <span className="text-cyan-300 shadow-neon-cyan/20 px-2 py-0.5 rounded bg-cyan-950/30 border border-cyan-900/50">{user?.email}</span>
            </p>
          </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center p-20 glass-effect border border-slate-800 rounded-2xl">
            ...
            <p className="mt-4 text-cyan-500 font-mono text-sm animate-pulse">ESTABLISHING DATABASE CONNECTION...</p>
          </div>
        ) : groups.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-16 glass-effect border border-cyan-500/20 rounded-2xl text-center relative overflow-hidden group">
            ...
            <h3 className="text-xl font-tech text-white mb-2">NO ACTIVE SQUADRONS FOUND</h3>
            <p className="text-slate-400 font-mono text-sm mb-8 max-w-md mx-auto">Initialize your first command group to deploy missions and monitor student operative readouts.</p>
            <Link 
              href="/group/create" 
            ...
            >
              INITIALIZE PROTOCOL <Play className="w-4 h-4 fill-current" />
            </Link>
          </div>
        ) : (
          <div className="space-y-4">
            <div className="flex items-center gap-3 px-2">
              ...
              <h2 className="text-sm font-mono font-bold text-slate-300 uppercase tracking-widest">Active Squadrons Deployed</h2>
                    ...
                    <div className="mt-auto pt-6">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between p-3.5 rounded-xl bg-slate-900/80 border border-slate-800 gap-3">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          >
                            {copied === g.access_code ? (
                              <span className="flex items-center gap-1"><CheckCircle2 className="w-3 h-3" /> SECURED</span>
                            ) : 'COPY KEY'}
                          </button>
                        </div>
                      </div>
...
fs.writeFileSync('app/(teacher)/dashboard/page.tsx', pageContent);
console.log('Teacher dashboard gamified!');
